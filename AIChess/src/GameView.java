...
public class GameView {
    ...
    public void init(final ChessBoard board) {
        this.board = board;
        frame = new JFrame("AI Chinese Chess");
        ...
        for (Map.Entry<String, Piece> stringPieceEntry : pieces.entrySet()) {
            ...
            int[] pos = stringPieceEntry.getValue().pos;
    ...
    class PieceOnClickListener extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            if (selectedPieceKey != null && key.charAt(0) != board.player) {
                int[] pos = board.pieces.get(key).pos;
                int[] selectedPiecePos = board.pieces.get(selectedPieceKey).pos;
                /* If an enemy piece already has been selected.*/
                for (int[] each : Rule.getNextMove(selectedPieceKey, selectedPiecePos, board)) {
    ...
    public void movePieceFromAI(String pieceKey, int[] to) {
        ...
        if (inNewPos != null) {
            pane.remove(pieceObjects.get(inNewPos.Info));
            pieceObjects.remove(inNewPos.Info);
        }
